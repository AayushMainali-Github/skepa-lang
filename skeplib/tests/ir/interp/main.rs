...
fn main() -> Int {
  ...
  let server: net.Socket = result.unwrapOk(net.accept(listener));
  ...
  let msg = result.unwrapOk(net.read(server));
  let host = option.unwrapSome(map.get(parts, "host"));
  let status = option.unwrapSome(map.get(response, "status"));
  let raw: Bytes = result.unwrapOk(net.readBytes(server));
  let exact: Bytes = result.unwrapOk(net.readN(server, 3));
  let local = result.unwrapOk(net.localAddr(client));
  let peer = result.unwrapOk(net.peerAddr(secure));

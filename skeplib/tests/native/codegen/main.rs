...
#[test]
fn codegen_builds_native_executable_for_net_http_get() {
    ...
    let server = thread::spawn(move || {
        ...
        assert!(request.contains("GET /hello HTTP/1.0"));
        stream
...
fn main() -> Int {{
  let body: String = net.httpGet("http://{addr}/hello");
  if ((body == "hello") && (str.len(body) == 5)) {{
    return 0;
  }}
  return 1;
}}
